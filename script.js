...
document.querySelectorAll(".section,.hero").forEach(el=>{
  ...
  // V45 fixed playlist — direct play inside the user click for Safari/iPhone
  const audioTimeV45 = (seconds) => {
  ...
  $$("[data-premium-player]").forEach((deck) => {
    ...
    const updateState = () => {
      ...
      current.textContent = audioTimeV45(elapsed);
      duration.textContent = audioTimeV45(total);

      const playing = !audio.paused && !audio.ended;
      deck.classList.toggle("is-playing", playing);

      if (status) {
        status.textContent = playing ? "NOW PLAYING" : "READY TO PLAY";
      }
    ...
    const setTrack = async (index, shouldPlay = false) => {
      ...
      if (!track.src) {
        showMessage(`Bài “${track.title}” chưa có đường dẫn MP3.`, "error");
        return;
      }

      const separator = track.src.includes("?") ? "&" : "?";
      audio.src = `${track.src}${separator}v=45`;
      audio.load();
      ...
      if (shouldPlay) {
        try {
          /*
           * This play() call runs directly inside the click handler.
           * That is important on iPhone/Safari, which can block delayed play().
           */
          await audio.play();
          ...
          if (error?.name === "NotAllowedError") {
            showMessage(
              `Đã chọn ${track.title}. Chạm nút Play một lần để Safari cho phép phát.`,
              "error"
            );
          } else {
            showMessage(
              `Không thể phát ${track.title}. Kiểm tra đúng file ${track.src}.`,
              "error"
            );
          }
        }
    ...
    tracks.forEach((trackButton, index) => {
      trackButton.addEventListener("click", async () => {
        if (index === activeIndex && audio.src) {
          try {
            if (audio.paused) {
              await audio.play();
            } else {
            ...
            showMessage(
              `Không thể phát bài này. Kiểm tra file ${trackButton.dataset.trackSrc}.`,
              "error"
            );
          }
    ...
    audio.addEventListener("error", () => {
      ...
      showMessage(
        `Không tìm thấy hoặc không đọc được ${track.src}. Tên file trên GitHub phải khớp tuyệt đối.`,
        "error"
      );
    ...
    // Initial track is already set in the native audio src attribute.
    applyTrackMeta(getTrack(0));
    updateState();
  });
