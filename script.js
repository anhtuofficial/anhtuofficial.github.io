...
document.querySelectorAll(".section,.hero").forEach(el=>{
  ...
  $$(".audio-file__controls[data-audio-player]").forEach((playerWrap) => {
    const audio = $(".audio-file__media", playerWrap);
    const playBtn = $("[data-audio-toggle]", playerWrap);
    const range = $("[data-audio-range]", playerWrap);
    const currentEl = $("[data-audio-current]", playerWrap);
    const durationEl = $("[data-audio-duration]", playerWrap);
    const shell = playerWrap.closest(".audio-file__shell");

    if (!audio || !playBtn || !range || !currentEl || !durationEl || !shell) return;

    const syncUI = () => {
      const dur = audio.duration || 0;
      const cur = audio.currentTime || 0;
      range.value = dur ? (cur / dur) * 100 : 0;
      currentEl.textContent = formatTime(cur);
      durationEl.textContent = formatTime(dur);
      const playing = !audio.paused;
      playerWrap.classList.toggle("is-playing", playing);
    ...
    range.addEventListener("input", () => {
      const dur = audio.duration || 0;
      if (!dur) return;
      audio.currentTime = (Number(range.value) / 100) * dur;
      syncUI();
    });
  ...
  // V37 premium audio player
  const v37FormatTime = (seconds) => {
  ...
  $$("[data-premium-player]").forEach((deck) => {
    ...
    const fileInput = $("[data-player-file]", deck);
    ...
    const setState = () => {
      ...
      seek.value = total ? (elapsed / total) * 100 : 0;
      current.textContent = v37FormatTime(elapsed);
      duration.textContent = v37FormatTime(total);

      const playing = !audio.paused;
      deck.classList.toggle("is-playing", playing);
      if (status) status.textContent = playing ? "NOW PLAYING" : "READY TO PLAY";
    };
    ...
    toggle.addEventListener("click", async () => {
      try {
        ...
        if (status) status.textContent = "AUDIO FILE NOT AVAILABLE";
      }
      setState();
    });
    ...
    fileInput?.addEventListener("change", async () => {
      ...
      if (status) status.textContent = "LOCAL FILE READY";
    ...
    audio.addEventListener("loadedmetadata", setState);
